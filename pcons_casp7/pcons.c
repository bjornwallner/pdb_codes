...
//	}
      ...
      for(i=0;i<files;i++)
	{
	  ...
	  for(j=i+1;j<files;j++)
	      ...
	      if(force_compare_all || !SameMethod(filenames[i],filenames[j]))
		{
		  ...
		  if(verbose)
		    printf("%d Comparing %s %s %s %s %lf %lf %lf %lf\n",total_number_of_comparisons,filenames[i],filenames[j],dm[i].filename,dm[j].filename,LG[0].LGscore,LG[0].Ssum,S_average1[i],S_average1[j]);
      ...
      if(files>1)
	{
	  ...
	  printf("REMARK LGscore parameters L=%d minsim=%lf factor=%lf step=%d\n",L,minsim,factor,step);
	  if(userdef_len>0)
	    {
	  ...
	  for(i=0;i<files;i++)
		...
		if(google)
		  ...
		  {
		    // printf("%s %5.3lf %5.3lf %5.3lf",filenames[i],(1-w1)*S_average[i]+w1*S_average1[i],S_average[i],S_average1[i]);
