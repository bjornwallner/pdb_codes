  ...
  if(argc==3)
    {
      ...
      //sep= strtod(argv[3],(char**)(argv[3]+strlen(argv[3])));
      cutoff=cutoff*cutoff;
      //  printf("%f %s %d\n",sep,argv[3],strlen(argv[2]));
		  ...
		  if(m[0].atm[j].rescount!=current_res_j)
		    {
		      ...
		      if(contacts[current_res_i][current_res_j]==0 && 
			 crd(m,i,j)<cutoff && 
			 abs(m[0].atm[i].rescount-m[0].atm[j].rescount)>=sep)
			{
			  //printf("%d %d %d %d %f %f\n",m[0].atm[j].rescount,m[0].atm[i].rescount,i,j,crd(m,i,j),crd(m,j,i));
...
//	}
    ...
    //}
      ...
      //printf("PFRMAT RR\n");
      //printf("TARGET %s\n",m[0].filename);
      // printf("REMARK CUTOFF %4.3lf\n",sqrt(cutoff));
      //printf("REMARK SEQUENCE SEPARATION %3d\n",sep);
      //printf("MODEL 1\n");
      printf("SEQ: %s\n",m[0].sequence);
      for(i=0;i<m[0].residues;i++)
	{
...
double crd(molecule *m,int atomno1, int atomno2)       /* CB distance, GLY CA Closed Residues Distance atomnoX is the first atom of the residue */
{
  int i,j;
  double dist,lowest_dist;
  lowest_dist=999999999999;
  for(i=atomno1;m[0].atm[i].rescount == m[0].atm[atomno1].rescount;i++)  
    {
      if(strcmp("CB ",m[0].atm[i].name)==0 ||
	 (strcmp("GLY",m[0].atm[i].residue)==0 && strcmp("CA ",m[0].atm[i].name)==0))
	{
	  for(j=atomno2;m[0].atm[j].rescount == m[0].atm[atomno2].rescount;j++)
	    {
	      if(strcmp("CB ",m[0].atm[j].name)==0 ||
		 (strcmp("GLY",m[0].atm[j].residue)==0 && strcmp("CA ",m[0].atm[j].name)==0)) 
		{
		  dist=distance(m,i,j);
		  //printf("%f ",dist);
		  if(dist<lowest_dist)
