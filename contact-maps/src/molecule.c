...
int  read_psipred2(char* filename,char* ss, double* coil,double* helix,double* sheet)
{
  //  char  *ss;
  char	buff[512];	/* Input string */
  char	line_flag[11];	
...
//Reads the higher precision profile created by makemat
void read_profile_mtx(char* filename,int (*profile)[22],char* seq)
{
  int i=0;
  int j=0;
  int line=0;
  ...
  if (fp!=NULL)
      ...
      while(fgets(buff,255,fp)!=NULL)
	{
	  ...
	  if(line == 2) { // get sequence
	    strncpy_NULL(seq,&buff[0],strlen(buff)-1);
	  }
	  if (!strncmp(buff, "-32768 ", 7))
	    {
	      if (sscanf(buff, "%*d%d%*d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%*d%d", &profile[j][ALA],  &profile[j][CYS], &profile[j][ASP],  &profile[j][GLU],  &profile[j][PHE], &profile[j][GLY],  &profile[j][HIS],  &profile[j][ILE],  &profile[j][LYS],  &profile[j][LEU],  &profile[j][MET],  &profile[j][ASN],  &profile[j][PRO],  &profile[j][GLN],  &profile[j][ARG],  &profile[j][SER],  &profile[j][THR],  &profile[j][VAL],  &profile[j][TRP],  &profile[j][TYR]) != 20)
		printf("Bad mtx format!\n");
	      j++;
...
int  read_stride(char *filename, char* ss)
{
  // char  *ss;
  char  *temp;
  char	buff[512];	/* Input string */
  ...
  if (fp!=NULL)
      ...
      while(fgets(buff,255,fp)!=NULL)
	{
	  ...
	  if(strcmp(line_flag,"SEQ")==0)
	    {
	      //printf("BUFF: %s\n",buff);
	      strncpy_NULL(str,&buff[10],50);
	      //sscanf(buff,"%s %d %s",tmp,&i,str);
	      //printf("%s",buff);
	      //	      printf("STR: %s\n",str);
	      //printf("LEN: %d\n",strlen(str));
	      strcat(seq,str);
...
double* calculate_parameters(char* pdbfile,char* psipred)
{
  ...
  //  {
	...
	{
	  printf("Different strlen!!!!fgfdgfdg %zu %zu\n%s\%s\n",strlen(psipred),strlen(ss),psipred,ss);
