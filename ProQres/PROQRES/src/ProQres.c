     ...
     char *argv[];
{
  ...
  //  int        prof_log_odds[MAXRES][22]={{}};
  double        prof[MAXRES][22]={{}};
  //  double        prof2[MAXRES][22]={{}};
  double        mean=0;
  double        helix[MAXRES];
  double        dist;
  ...
  char psipred[MAXRES];
  char          profseq[MAXRES];
  char          profseq2[MAXRES];
  char          proffile[1500]="undef";
  char          proffile_mtx[1500]="undef";
  char          line_flag[10];
  char          buff[1000];
  char          basename[200]="undef";
  //  char          tmp_str[100000];
  ...
  while(i<argc)
      ...
      else if (strcmp(argv[i],"-prof")==0)
	{
      ...
      else if (strcmp(argv[i],"-prof_mtx")==0)
	{
	  strcpy(proffile_mtx,argv[i+1]);
	  i++;
	}
	...
	{  
      ...
      else if(strcmp(argv[i],"-output_atom")==0)
	{  
	  output_atom=1;
	}
        else if(strcmp(argv[i],"-output_input")==0)
	{  
	  output_input=1;
	}
  ...
  printf("#output atom parameters: %d\n",output_atom);
  printf("#output input parameters: %d\n",output_input);
  ...
  strcpy(net1,"/afs/pdc.kth.se/home/b/bjornw/source/c/pdb/ProQres/weights");
  if(getenv("PROQRESDIR")!=NULL) {
    ...
    {
      if(!output_atom) {
	printf("#ENV variable PROQRESDIR not set will use the default...\n");
      }
  ...
  /*
    ...
    {
      if(!output_atom) {
	fprintf(stderr,"ENV variable PROQRESDIR not set will use the default...\n");
	//	exit(1);
      }
      strcpy(net1,"/afs/pdc.kth.se/home/b/bjornw/source/c/pdb/ProQres/weights");
    }
  ...
  if(!output_atom) {
    read_stride(stridefile,stride);
    read_psipred2(psipredfile,psipred,coil,helix,sheet);
  ...
  if(read_molecules(m,'a')==0)
	...
	{
	  printf("#Reading prof...\n");
	  //	  printf("%s %s\n",proffile,proffile_mtx);
	  read_profile(proffile,prof,profseq);
	  //	  read_profile_mtx(proffile,prof_log_odds,profseq2);
	  ...
	  for(i=0;i<strlen(profseq);i++)
	      ...
	      for(j=0;j<20;j++)
		{
		  ...
		  //prof[i][j]=prof2[i][j];
		  //		  prof[i][j]=1/(1+exp((double)(-(double)prof_log_odds[i][j]/100))); //(double)1/(1+exp(-(double)prof_log_odds[i][j]/100));
		  //	  printf("%lf ",prof[i][j]);
		    // printf("1:%d %lf %lf %lf\n",prof_log_odds[i][j],exp((double)(-(double)prof_log_odds[i][j]/100)),prof[i][j],prof2[i][j]);
		  if(prof[i][j]!=0)
		    check=1;
		}
	      //	      printf("\n");
      ...
      else //initialize prof and profseq to standard values
	{
      ...
      res_contact_map = malloc(MAXRES*sizeof(int *));
      for(i=0;i<MAXRES;i++)
	res_contact_map[i] = malloc(MAXRES*sizeof(int));
      ...
      for(i=0;i<m[0].atoms;i++)  
	{
      ...
      //exit(1);
      if(output_atom || res_in_surffile == m[0].residues && strlen(psipred) == strlen(stride) && strlen(stride)==m[0].residues)
	{
	  for(i=-(windowsize-1)/2;i<m[0].residues-(windowsize-1)/2;i++)
		...
		if(stride[current_res_i] == 'H')
	      ...
	      if(output_atom) { 
		printf("POS: %-10d ",current_res_i+1);
		 for(k=0;k<91;k++,l++)
		   printf("%8.6f ",atomfrac[k]);
		 printf("\n");
	      } else if(output_input) {
		 printf("POS: %-10d\n", current_res_i+1);
		 printf("ATOM: ");
		 for(k=0;k<91;k++)
		   printf("%8.6f ",atomfrac[k]);
		 printf("\n");
		 printf("RES: ",number_of_residuetypes);
		 for(k=0;k<(number_of_residuetypes*(number_of_residuetypes+1)/2);k++)
		   printf("%8.6f ",resfrac[k]);
		 printf("\n");
		 
		 printf("SURF25: ");
		 for(k=0;k<6;k++)
		   printf("%8.6f ",surface25_prof[k]);
		 printf("\n");
		 printf("SURF50: ");
		 for(k=0;k<6;k++)
		   printf("%8.6f ",surface50_prof[k]);
		 printf("\n");
		 printf("SURF75: ");
		 for(k=0;k<6;k++)
		   printf("%8.6f ",surface75_prof[k]);
		 printf("\n");
		 printf("SURF100: ");
		 for(k=0;k<6;k++)
		   printf("%8.6f ",surface100_prof[k]);
		 printf("\n");
		 printf("SS: ");
		 printf("%8.6f\n",ss);
		...
		printf("%6d %c %8.5lf\n",i+(windowsize-1)/2+1,m[0].sequence[i+(windowsize-1)/2],predicted_quality);
		 ...
		 atomfrac[k]=0;
	      ...
	      for(k=0;k<number_of_residuetypes;k++){
		for(l=0;l<number_of_residuetypes;l++){
		  res[k][l]=0;
		}
		surface25[k]=0;
		surface50[k]=0;
