  ...
  if(read_molecules(m,'a')==0)
      ...
      if(res_in_surffile == m[0].residues && strlen(psipred) == strlen(stride) && strlen(stride)==m[0].residues)
	{
	  for(i=-(windowsize-1)/2;i<m[0].residues-(windowsize-1)/2;i++)
		...
		printf("%lf ",resfrac[k]);
		tmp_sum+=resfrac[k];
		sprintf(tmp_short_str,"%lf ",resfrac[k]);
		//printf("%s %s\n",tmp_short_str,tmp_str);
		strcat(tmp_str,tmp_short_str);
	      }
	      strcat(tmp_str,"\nATOM: ");
	      for(k=0;k<91;k++){
		printf("%lf ",atomfrac[k]);
		sprintf(tmp_short_str,"%lf ",atomfrac[k]);
		strcat(tmp_str,tmp_short_str);
	      }
	      strcat(tmp_str,"\nATOM3: ");
	      for(k=0;k<6;k++){
		//printf("%lf ",atomfrac3[k]);
		sprintf(tmp_short_str,"%lf ",atomfrac3[k]);
		strcat(tmp_str,tmp_short_str);
	      }
	      //for(k=0;k<21;k++)
	      //	printf("%lf ",resfrac[k]);
		...
		max_k=20;
	      for(k=0;k<max_k;k++){
		printf("%lf ",surface25_prof[k]);
		sprintf(tmp_short_str,"%lf ",surface25_prof[k]);
		strcat(tmp_str,tmp_short_str);
	      }
	      for(k=0;k<max_k;k++){
		printf("%lf ",surface50_prof[k]);
		sprintf(tmp_short_str,"%lf ",surface50_prof[k]);
		strcat(tmp_str,tmp_short_str);
	      }
	      for(k=0;k<max_k;k++){
		printf("%lf ",surface75_prof[k]);
		sprintf(tmp_short_str,"%lf ",surface75_prof[k]);
		strcat(tmp_str,tmp_short_str);
	      }
	      for(k=0;k<max_k;k++){
		printf("%lf ",surface100_prof[k]);
		sprintf(tmp_short_str,"%lf ",surface100_prof[k]);
		strcat(tmp_str,tmp_short_str);
	      }
	      sprintf(tmp_short_str,"%lf ",ss);
	      strcat(tmp_str,"\nSS: ");
	      strcat(tmp_str,tmp_short_str);
	      printf("%lf\n",ss);
	      printf("%s\n",tmp_str);
