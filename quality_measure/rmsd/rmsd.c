 ...
 if (files==2)	/* Valid input line? */
   {
	 ...
	 if(matrix_flag)
	   {
	     //  printf("CA: ");
	     printf("%lf %lf %lf\n%lf %lf %lf\n%lf %lf %lf\n",s_ca[0][0],s_ca[0][1],s_ca[0][2],s_ca[1][0],s_ca[1][1],s_ca[1][2],s_ca[2][0],s_ca[2][1],s_ca[2][2]);
	   }
	 else
	   {
	     output_results2();
    ...
    molecule 	*m1,*m2;		
{
  ...
  for(i=0;i<m1->atoms;i++)
      ...
      if(strcmp(m1->atm[i].name,m2->atm[i].name)!=0) //inconsistency do a move to correct
	{
	  ...
	  // printf("BEFORE %d %d\nBEFORE: %3s %3s %3s %3s\n",i,j,m1->atm[i].name,m1->atm[i].residue,m2->atm[j].name,m2->atm[j].residue);
	  // printf("BEFORE: %3s %5d %5d %8.3lf%8.3lf%8.3lf\n",m2->atm[j].name,m2->atm[j].resnum,m2->atm[j].number,m2->atm[j].x,m2->atm[j].y,m2->atm[j].z);
...
int center_molecule(molecule *m)	/* Center molecule molecules to be superimposed */
     //    molecule *m;
{
  ...
  if(matrix_flag)
    {
      printf("CENTER natoms: %d %lf %lf %lf \n",natoms,xcen,ycen,zcen);
    }
  for (i=0;i<m->atoms;i++)
    {
      m->atm[i].x-=xcen;
