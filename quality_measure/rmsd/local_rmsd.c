    ...
    char *argv[];
{
 ...
 int    central_residue=0;
 double	s[3][3];	/* Final transformation matrix */
 double s_all[3][3];
 double s_ca[3][3];
 double rms_vec[MAXATMS]={};
 double rms_sum=0;
 double rms_overall=0;
 int natoms=0;
 ...
 if (files==2)	/* Valid input line? */
   {
	 ...
	 //	 printf("%d\n",winsize);
	 //exit(1);
	 for (i=0;i<m[0].atoms;i++){ // deselect all atoms
	  m[0].atm[i].selected=FALSE;
	  m[1].atm[i].selected=FALSE;
	}
	 //select a window
	 
	 for(j=0;j<(winsize-1)/2;j++)
	 {
	   printf("%-5d %s %lf\n",m[1].atm[j].resnum,m[1].atm[j].residue,99.0);
	   rms_vec[j]=99;
	 }
	 for(i=0;i<=m[0].atoms-winsize;i++)
	   {
	     for (j=i;j<i+winsize;j++){
	//printf("%d \t %s \t %3s %4d \t %8.4lf -- ",
	//	      m[0].atm[j].number,m[0].atm[j].name,
	//	      m[0].atm[j].residue,m[0].atm[j].resnum,
	//	      sqrt(m[0].atm[j].rms));
	//printf("%d \t %s \t %3s %4d \t %8.4lf\n",
	//	      m[1].atm[j].number,m[1].atm[j].name,
	//	      m[1].atm[j].residue,m[1].atm[j].resnum,
	//	      sqrt(m[1].atm[j].rms));
	       
	       m[0].atm[j].selected=TRUE;
	       m[1].atm[j].selected=TRUE;
	     }
	     central_residue=i+(winsize-1)/2;
	     center_molecule(&m[0]);
	      center_molecule(&m[1]);
	      rms=superimpose_molecules(&m[0],&m[1],s_ca);
	      
	      printf("%-5d %s %lf\n",m[1].atm[central_residue].resnum,m[1].atm[central_residue].residue,rms);
	      rms_vec[central_residue]=rms;
	      rms_sum+=rms*rms;
	      natoms++;
	 ...
	 central_residue++;
	 for(j=0;j<(winsize-1)/2;j++,central_residue++)
	 {
	   printf("%-5d %s %lf\n",m[1].atm[central_residue].resnum,m[1].atm[central_residue].residue,99.0);
	   rms_vec[central_residue]=rms;
	   
	 }
	 printf("Root Mean Square: %15.10f\n",sqrt(rms_sum/natoms));
	 //rms_overall=sqrt(rms_sum/natoms);
	 ...
	 // if (output_flag_ca)
	 //  {
	     //read_second_molecule(&m[0],&m[1],s_ca);
   ...
   printf("Usage: rmsd [ -v ] [ -o outfile] [ -ca outfile] file1 file2\n");
}
    ...
    double 		s[3][3];	/* Final transformation matrix */
{
    ...
    double 		error,error2;		/* Final superimposition error */
