...
int read_molecules(molecule *m, char *chain);
  ...
  if(argc==4)
    {
      ...
      cutoff=strtod(argv[2],NULL); //strtod(argv[3],(char**)(argv[3]+strlen(argv[2])));
    ...
    {
      printf("Usage: contact_map [pdb_file] cutoff chain\n");
      exit(1);
    }
  if(read_molecules(m,chain)==0)
    {  
      //  printf("%s\n",m[0].sequence);
      //exit(1);
...
int read_molecules(molecule *m, char *in_chain)	/* Reads in molecules to be superimposed */
{
  ...
  if(strcmp(in_chain,"_")==0)
    {
      // printf("changed chain\n");
      in_chain=" ";
    }
		  ...
		  if(strcmp(old_resname,resname)!=0)
		    {
		      ...
		      //printf("%s %s %s\n",resname,residue,m[0].sequence);
		    }
		  //sscanf(&buff[22],"%d %lf %lf %lf",&resnum,&x,&y,&z);
		  //printf("test %d %s %s %s %d %lf %lf %lf\n",number,name,residue,chain,resnum,x,y,z);
		  m[0].atm[atoms].x=x;
		  m[0].atm[atoms].y=y;
		  m[0].atm[atoms].z=z;
  ...
  if(m[0].residues==0)
    {
      fprintf(stderr,"No atoms for the specified chain \"%s\"\n",in_chain);
      return(1);
    }
